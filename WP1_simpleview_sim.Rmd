...
*Can also run version with tests for non-normality, but probably not an issue in full dataset for this analysis? Might be in poor comprehender LPA though.* PT - The model can cope with some amount of departures from normality on individual items, it only has the assumption of multivariate normality conditional on class. ITs likely that skew and floor/ceiling effects will be tricky.
...
... haven't got feel for whether/when to add further covariances? PT - if you're not sure and can't justify it, then I would stop and assess whether the model structure is sufficient. Looking at the current model fit, it already looks very good, so further additions should be added if theoretically plausible and sensible.
...
m_cfa2c <- update(m_cfa2b,
                 TITLE = ~ "Confirmatory Factor Analysis - Two-Factor - modification2 word covariance;",
                 MODEL = ~. + "nonwAcc with	wordAcc;")

m_cfa2c_fit <- mplusModeler(m_cfa2c,
                           modelout = "./mod_scripts/cfa/sv_cfa2c.inp",
                           check = TRUE, run = TRUE)
...
# Inspect model (by = loadings, with = covariances)
cfa_models$sv_cfa2c.out$parameters$unstandardized
cfa_models$sv_cfa2c.out$parameters$stdy.standardized

# Inspect modification indices
cfa_models$sv_cfa2c.out$mod_indices
```

```{r cfa-diagram2}
semPlotModel(cfa_models$sv_cfa2c.out)
semPaths(semPlotModel(cfa_models$sv_cfa2c.out, mplusStd = "stdyx"), what = "paths", whatLabels = "std", rotation = 2, intercepts = FALSE)
```
...
m_fmm2 <- mplusObject(
  TITLE = "Factor Mixture model - 2 classes;",
  VARIABLE = "classes = c(2);",
  ANALYSIS = "estimator = mlr; type = mixture; stiterations = 50; starts = 250 25;    
                   lrtbootstrap = 200; lrtstarts = 25 5 10 25;",                  # check estimator with PT
  MODEL = " %OVERALL% 
  ...
  nonwAcc with	naraAcc;
  wordAcc with	naraAcc;", 
  OUTPUT = "sampstat; TECH10; TECH11; TECH14; ",
  PLOT = "TYPE = PLOT3;",
  usevariables = colnames(sim_dat_NA[,!names(sim_dat_NA) %in% c("id","k")]),
  rdata = sim_dat_NA)

m_fmm_fit2 <- mplusModeler(m_fmm2,
                           modelout = "./mod_scripts/fmm/sv_fmm2.inp",
                           check = TRUE, run = TRUE)
...
m_fmm3 <- mplusObject(
  TITLE = "Factor mixture model - 3 classes;",
  ...
  nonwAcc with	wordAcc;
  nonwAcc with	naraAcc;
  wordAcc with	naraAcc;",   # single reading factor
  OUTPUT = "sampstat; TECH10; TECH11; TECH14; ",
  PLOT = "TYPE = PLOT3;",
  usevariables = colnames(sim_dat_NA[,!names(sim_dat_NA) %in% c("id","k")]),
  rdata = sim_dat_NA)

m_fmm_fit3 <- mplusModeler(m_fmm3,
                           modelout = "./mod_scripts/fmm/sv_fmm3.inp",
                           check = TRUE, run = TRUE)

#==============================================================================#

m_fmm4 <- mplusObject(
  TITLE = "Factor mixture model - 4 classes;",
  VARIABLE = "classes = c(4);",
  ANALYSIS = "estimator = mlr; type = mixture; stiterations = 50; starts = 250 25;    
                   lrtbootstrap = 200; lrtstarts = 25 5 10 25;",                  # check estimator with PT
  MODEL = " %OVERALL% 
  acc by naraAcc wordAcc nonwAcc; 
...
m_fmm_fit4 <- mplusModeler(m_fmm4,
                           modelout = "./mod_scripts/fmm/sv_fmm4.inp",
                           check = TRUE, run = TRUE)

#==============================================================================#

m_fmm5 <- mplusObject(
  TITLE = "Factor mixture model - 5 classes;",
  VARIABLE = "classes = c(5);",
  ANALYSIS = "estimator = mlr; type = mixture; stiterations = 50; starts = 250 25;    
                   lrtbootstrap = 200; lrtstarts = 25 5 10 25;",                  # check estimator with PT
  MODEL = " %OVERALL% 
  acc by naraAcc wordAcc nonwAcc; 
...
m_fmm_fit5 <- mplusModeler(m_fmm5,
                           modelout = "./mod_scripts/fmm/sv_fmm5.inp",
                           check = TRUE, run = TRUE)

#==============================================================================#

fmm_model_out <- readModels(target = "./mod_scripts/fmm", filefilter = "sv_fmm")
fmm_mod_summary <- mixtureSummaryTable(fmm_model_out)
...
### Inspecting resultant LPA model
...
m_lpa_select_fit <- mplusModeler(m_lpa_extract, modelout = "./mod_scripts/lpa/sv_lpa_select.inp", run = TRUE)

m_lpa_select_out <- readModels("./mod_scripts/lpa/sv_lpa_select.out")
