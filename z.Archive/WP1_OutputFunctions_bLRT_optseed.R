...
# mm_bLRT_seed: refit subset of models to save out participant classifications ----
mm_bLRT_seed <- function(orig_mods = NA,        # list of original models (in environment)
                            orig_output = NA, 
  ...
  # Update scripts 
    for (model in 1:length(orig_mods)){
      ...
      mplusModeler(body, sprintf("%s/%s_%s_optseed_%dclass.dat", filepath, analysis_id, model_name, n_classes), run = TRUE)
  }
  filefilter = paste0(model_name, "_optseed")
