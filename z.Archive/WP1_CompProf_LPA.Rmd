---
title: 'WP1: Profiles of comprehension difficulty - Latent Profile Analysis'
output: 
  html_document:
    toc: true
    toc_float: true
...
This script is used for running the latent profile models. It will only be used *if* the CFA indicates that a single factor is best, otherwise we will use a factor mixture model (WP1_CompProf_FMM.Rmd).
...
if(dir.exists("./mplus_models/lpa")==FALSE){dir.create("./mplus_models/lpa")} 
...
Options reduced to save time for now, but change for actual modelling:
starts - 500 50
lrtbootstrap 50
...
m_lpa_base <- mplusObject(
  ...
  MODEL = "%OVERALL% 
           ...
           cntSpan on f10age;", 
  OUTPUT = "sampstat; stdyx; residual; TECH1; TECH7; TECH8; TECH11; TECH14;",
  PLOT = "TYPE = PLOT3;",
  usevariables = colnames(cp_data[,!names(cp_data) %in% c("yp_id", "yp_no", "k")]),
  rdata = cp_data)

# Specify all variances
lpa_vars <- "naraComp; naraAcc; wordAcc; nonwAcc; woldComp; naraRate; 
...
lpa_covars <- "
  ...
  naraComp with naraRate;
  naraComp with woldVcb;
  naraComp with wiscVcb;
  naraComp with wiscPcmp;
  naraComp with wiscCode;
  naraComp with wiscParr;
...
mplusModeler(m_lpa_benchmark, "./mplus_models/lpa/cp_lpa_mbenchmark.dat", run = TRUE)

# Extract parameters for the benchmark model
m_lpa_benchmark <- readModels(target = "./mplus_models/lpa", filefilter = "mbenchmark") %>% 
...
m_lpa_a <- lapply(1:6, function(k) {
   ...
   mplusModeler(body, sprintf("mplus_models/lpa/cp_lpa_a_%dclass.dat", k), run = TRUE)
 })
```
...
```{r lpa-a-output}
# Read in output
lpa_a_out <- readModels(target = "./mplus_models/lpa", filefilter = "cp_lpa_a")
...
m_lpa_b <- lapply(1:6, function(k) {
   ...
   # Run model
   mplusModeler(body, sprintf("mplus_models/lpa/cp_lpa_b_%dclass.dat", k), run = TRUE)
 })
```
...
```{r lpa-b-output}
# Read in output
lpa_b_out <- readModels(target = "./mplus_models/lpa", filefilter = "cp_lpa_b")
...
m_lpa_c <- lapply(1:6, function(k) {
  ...
  # Run model
  mplusModeler(body, sprintf("mplus_models/lpa/cp_lpa_c_%dclass.dat", k), run = TRUE)
 })
...
```{r lpa-c-output}
# Read in output
lpa_c_out <- readModels(target = "./mplus_models/lpa", filefilter = "cp_lpa_c")
...
m_lpa_d <- lapply(1:6, function(k) {
  ...
  # Run model
  mplusModeler(body, sprintf("mplus_models/lpa/cp_lpa_d_%dclass.dat", k), run = TRUE)
 })
```
...
```{r lpa-d-output}
# Read in output
lpa_d_out <- readModels(target = "./mplus_models/lpa", filefilter = "cp_lpa_d")
...
m_lpa_e <- lapply(1:6, function(k) {
  ...
  # Run model
  mplusModeler(body, sprintf("mplus_models/lpa/cp_lpa_e_%dclass.dat", k), run = TRUE)
 })
```
...
```{r lpa-e-output}
# Read in output
lpa_e_out <- readModels(target = "./mplus_models/lpa", filefilter = "cp_lpa_e")
