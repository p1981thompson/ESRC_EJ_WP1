...
*If* the CFA indicates that we should proceed with a factor mixture model, consider parameters that would improve model fit in line with theory. 
...
*Should specify conditions under which do LPA vs. FMM*
...
output_initial_fit <- function(model_name = NULL){
  ...
  lpa_summaries <- mixtureSummaryTable(lpa_out, keepCols = c("Title", "Classes", "Parameters", "LL", "AIC", "BIC", "aBIC", "T11_VLMR_PValue", "T11_LMR_PValue", "BLRT_PValue")) %>% 
    mutate(BF = exp(0.5*(BIC-lead(BIC))))  # Compute approximate BF

  # Print indices used for model selection
  print(lpa_summaries %>% 
    select(Title, Classes, Parameters, LL, T11_VLMR_PValue, T11_LMR_PValue, BLRT_PValue, BF) %>% 
    arrange(Title) %>% 
...
**d) Approximate BF (fewest classes with moderate-strong evidence comparied to next model)**: 2-class
...
**d) Approximate BF (fewest classes with moderate-strong evidence compared to next model)**: 1
