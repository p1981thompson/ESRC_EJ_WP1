...
* NARA comp - range 0-45, M = 25 - STANDARDISED
* NARA acc (errors) - range 0-100, M = 66  (note that a lower score is better) - STANDARDISED
* Word and nonword acc - range 0-10 
* WOLD comp - range 2-14

Create 5 classes, based on those from Torppa et al. (2007).

```{r measures}

################# CLASS 1 MEASURES ################### - Average readers
...
################# CLASS 5 MEASURES ################### - Poor comprehenders
...
stddev4 <- c(nara_comp_sd4, nara_acc_sd4, word_acc_sd4, nonword_acc_sd4, wold_comp_sd4, age_months_f8_sd, age_months_f9_sd)
stddev5 <- c(nara_comp_sd5, nara_acc_sd5, word_acc_sd5, nonword_acc_sd5, wold_comp_sd5, age_months_f8_sd, age_months_f9_sd)
...
covar4 <- sdcor2cov(stddev4, corr)
covar5 <- sdcor2cov(stddev5, corr)
...
means4 <- c(nara_comp_m4, nara_acc_m4, word_acc_m4, nonword_acc_m4, wold_comp_m4, age_months_f8_m, age_months_f9_m)
means5 <- c(nara_comp_m5, nara_acc_m5, word_acc_m5, nonword_acc_m5, wold_comp_m5, age_months_f8_m, age_months_f9_m)

# Number of observations
n_ppts <- 6943

# Latent profile probabilities
class_probs<-c(0.45, 0.15, 0.1, 0.2, 0.1)

# Generate mixing profile indicator based on profile probabilities
k<-sample(1:5,n_ppts,class_probs,replace=TRUE)
...
for(i in 1:n_ppts){
sim_dat[i,]<-switch(k[i],mvrnorm(n = 1, mu = means1, Sigma = covar1), mvrnorm(n = 1, mu = means2, Sigma = covar2), mvrnorm(n = 1, mu = means3, Sigma = covar3), mvrnorm(n = 1, mu = means4, Sigma = covar4), mvrnorm(n = 1, mu = means5, Sigma = covar5))
}
...
filename <- paste0("sim_data/WP1_SimpView_sim_split_n", n_ppts, "_k", length(class_probs), ".csv")
