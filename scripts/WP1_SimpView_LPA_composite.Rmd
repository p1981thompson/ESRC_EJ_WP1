...
# m_lpa_a_starts <- lapply(c(9), function(k) {
#    body <- update(m_lpa_base,
#      TITLE = as.formula(sprintf("~ 'LPA Model A: %d classes;'", k)),
#      VARIABLE = as.formula(sprintf("~ 'idvariable = cidb3153; classes = c(%d);'", k)),
#      MODEL = as.formula(sprintf("~ . + '%s %s'", lpa_vars, no_covars)),
...
lpa_a_rerun <- mm_extract_data(orig_mods = m_lpa_a, orig_output = lpa_a_out, 
                               candidate_mods = c(),                           # change models to best candidates
...
```{r lpa-a-selected}
# lpa_a_final_nclass <-             # change model to best candidate
# lpa_a_final_m <- lpa_a_rerun[]    # change model to best candidate
```
...
lpa_b_summary[c(6, 8, 9), c(3, 5:11)] <- NA  # Could also remove values from non-trusted models? 
lpa_b_summary
...
```{r lpa-b-selected}
# lpa_b_final_nclass <-            # change model to best candidate
# lpa_b_final_m <- lpa_b_rerun[]   # change model to best candidate
```
...
# m_lpa_c_starts <- lapply(c(6, 9), function(k) {
#   
#   # Update model spec 1: overall model estimates (variances only)
#   body <- update(m_lpa_base,
#               MODEL = as.formula(sprintf("~ . + '%s %s'", lpa_vars, no_covars)))
...
lpa_c_summary[c(9), c(3, 5:11)] <- NA  # Could also remove values from non-trusted models? 
lpa_c_summary
...
```{r lpa-c-selected}
# lpa_c_final_nclass <-            # change to best model
# lpa_c_final_m <- lpa_c_rerun[]   # change to best model
```
...
```{r lpa-d-selected}
# lpa_d_final_nclass <-             # change to selected model
# lpa_d_final_m <- lpa_d_rerun[]    # change to selected model
```
...
# m_lpa_e_starts <- lapply(c(6, 7), function(k) {
#   
#   # Update model spec 1: overall model estimates (variances and covariances)
#   body <- update(m_lpa_base,
#               MODEL = as.formula(sprintf("~ . + '%s \n %s '", lpa_vars, lpa_covars)))   
...
lpa_e_summary[c(10), c(3, 5:11)] <- NA  # Could also remove values from non-trusted models?
lpa_e_summary
...
```{r lpa-e-selected}
# lpa_e_final_nclass <-            # change to best model
# lpa_e_final_m <- lpa_e_rerun[]   # change to best model
```
