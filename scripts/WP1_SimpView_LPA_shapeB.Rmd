...
# Variances specified differently for different models below
...
m_fmm3 <- lapply(1:6, function(k) {
  ...
  body <- update(body,
     ...
     SAVEDATA = as.formula(sprintf(" ~ 'FILE IS fmm3_%dclass.dat; SAVE = cprobabilities;'", k)))
...
# Read in output
fmm3_out <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "(fmm3_)([1-9])")
...
m_fmm3_starts <- lapply(c(3:4), function(k) {
  ...
  # Run model
  mplusModeler(body, sprintf("mplus_models/simpview/shapeB/fmm3_lmr_%dclass.dat", k), run = TRUE)
 })
```
...
**Note - still some issues with 3-/4-class models and local maxima, but stopped trying to re-run for time being as clear that this set of models not as good a fit as ones below**
...
```{r fmm3-output}
# Read in output
fmm3_out2 <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "(fmm3_lmr_)([1-9])")

# Print table 
fmm3_summary <- fmm_enum_table(output = fmm3_out2)
fmm3_summary
...
m_cfa_fit <- mplusModeler(m_cfa,
                           modelout = "./mplus_models/simpview/shapeB/cfa_scores.inp",
...
# Fit model with additional covariance between the comprehension measures (accuracy also acceptable)
...
m_lpa4 <- lapply(1:8, function(k) {
  ...
  #mplusModeler(body, sprintf("mplus_models/simpview/shapeB/lpa4_%dclass1S.dat", k), run = TRUE)
 })
...
# m_lpa4_starts <- lapply(c(8), function(k) {
# 
#   # Update model spec 1: overall model estimates (variances and covariances)
#   body <- update(m_base,
#               MODEL = as.formula(sprintf("~ . + '%s \n %s
...
# 
#   # Run model
#   mplusModeler(body, sprintf("mplus_models/simpview/shapeB/lpa4_%dclass.dat", k), run = TRUE)
#  })
```
...
# if(dir.exists("./mplus_models/simpview/shapeB/nonopt")==FALSE){dir.create("./mplus_models/simpview/shapeB/nonopt")} 
# 
...
# }
# 
# all_files <- list.files("./mplus_models/simpview/shapeB/", pattern = "^lpa4.*out$")
# all_files
...
#             old_folder = "./mplus_models/simpview/shapeB",
#             new_folder = "./mplus_models/simpview/shapeB/nonopt")
# }
...
**7-class LMR test not quite accurate - but only ~50 out on LL; doesn't affect decision**
...
```{r lpa4-output}
# Read in output
lpa4_out2 <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "(lpa4_lmr_)([1-9])")
...
| **d) Approximate BF** (fewest classes with moderate-strong evidence compared to next model): *NA/6-class*
| **e) Approximate correct model probability:** *NA/6-class*
...
# Inspect transitions 
extract_classes(lpa4_cand[3:4], type = "lpa")
```
...
-   **Interpretability:** 4-class model still mostly differentiated by level. 5-class+ models show further differentiation by shape. In comparison to models fitted to first half of sample - profiles look comparable in 6-class model, but not in 5-class model. 
