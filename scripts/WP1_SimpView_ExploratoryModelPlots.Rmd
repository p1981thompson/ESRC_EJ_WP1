...
final_mod <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "lpa4_rerun_5class")  # final model file
...
ppt_data <- read.csv("../data/processed/WP1_data_subB.csv") %>%
...
plot_data <- std_data %>% 
  #mutate(c = recode_factor(c, `1` = 1, `2` = 4, `3` = 2, `4` = 3, `5` = 5, `6` = 6)) %>% 
...
ggsave("../output/figures/lpa4_B_5class_desc.tiff", dpi = 600, width = 7, height = 5)
...
#ggsave("../output/figures/lpa4full_6class_raw.tiff", dpi = 600, width = 7, height = 5)
...
#ggsave("../output/figures/lpa4full_6class_itemXcomp.tiff", dpi = 600, width = 7, height = 5)
...
ggplot(std_data, aes(nara_acc_bstd_f9, nara_comp_bstd_f9, color = as.factor(c))) + 
  ...
  ylab("Reading comprehension")
#ggsave("../output/figures/lpa4full_6class_passXcomp.tiff", dpi = 600, width = 7, height = 5)
```
...
ppt_data <- read.csv("../data/processed/WP1_data_all.csv") %>%
  select(cidB3153, age_m_f8, age_m_f9, nara_comp_raw_f9, nara_acc_raw_f9, read_word_raw_f9, read_nonw_raw_f9, wold_comp_raw_f8,
         nara_comp_std_f9, nara_acc_std_f9, wisc_vcb_raw_f8, wold_vcb_raw_f8) %>% 
  mutate(read_comb_raw_f9 = (read_word_raw_f9 + read_nonw_raw_f9))
...
std_data <- sample_std(data = ppt_classes, test_scores = c("read_comb_raw_f9", "nara_acc_raw_f9", "nara_comp_raw_f9"), age = "age_m_f9")
std_data <- sample_std(data = std_data, test_scores = c("wold_comp_raw_f8", "wisc_vcb_raw_f8", "wold_vcb_raw_f8"), age = "age_m_f8")
...
class_averages_std <- std_data %>% 
  group_by(c) %>% 
  summarise_at(vars(wisc_vcb_bstd_f8, wold_vcb_bstd_f8), 
               list(mean = mean, sd = sd, min = min, max = max), na.rm = TRUE) %>% 
  round(.,2)
