...
This script is used for extracting descriptive statistics for the final class model.
...
class_averages_std <- std_data %>% 
  group_by(c) %>% 
  summarise_at(vars(read_comb_bstd_f9, nara_acc_std_f9, nara_comp_std_f9, wold_comp_bstd_f8), 
               list(mean = mean, sd = sd, min = min, max = max), na.rm = TRUE) %>% 
  round(.,2)
...
ggplot(std_data, aes(nara_acc_bstd_f9, nara_comp_bstd_f9, color = as.factor(c))) + 
  ...
  ylab("Reading comprehension")
ggsave("../output/figures/SimpView_E4_passXcomp.tiff", dpi = 600, width = 7, height = 5)
```
