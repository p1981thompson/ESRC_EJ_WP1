---
title: 'WP1: Simple View - Focusing on shape over level - Subsample B'
output: 
  html_document:
    toc: true
    toc_float: true
...
The models are based on those proposed by [Morin & Marsh (2015)](https://www.tandfonline.com/doi/pdf/10.1080/10705511.2014.919825). The models were initially run on the "exploratory" or "calibration" half of the dataset (subsample A; **WP1_SimpView_LPA_shape.Rmd**), but the best model selected showed a significantly poorer fit when fitted with subsample B (**WP1_SimpView_ShapeValidation.Rmd**)---although the profiles looked similar in shape. This script repeats the class enumeration process with subsample B, to ensure that we have selected the best model specification overall.
...
```{r output-functions, message=FALSE}
...
```{r load-data}
sv_data <- read.csv("../data/processed/WP1_data_subB.csv")  %>%       # ALSPAC data
...
Model 3 is a factor mixture model with a class-invariant higher order latent factor. The class-invariance of the factor allows for a measure of global ability across all measures, defined in the same way across all classes. We allowed for correlated errors between the comprehension variables in the global ability factor.
...
m_fmm3 <- lapply(c(1:8), function(k) {
  ...
  # Run model
  mplusModeler(body, sprintf("mplus_models/simpview/shapeB/fmm3_%dclass.dat", k), run = FALSE)
 })

# Read in output
fmm3_out <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "(fmm3_)([1-8])")
```
...
```
*Note that the 6-class model LL was not replicated (tried up to 8000 2000 starts)*
...
fmm3_lmr <- recomp_LMR(orig_mods = m_fmm3, 
                        ...
                        #mods = c(7),
                        #kstarts = "500 50",
                        filepath = "mplus_models/simpview/shapeB",
                        rerun = FALSE
                        )
...
```
*The k-1 model for the 5-class LMR test did not match the LL achieved by the 4-class model initially fit, but this could not be resolved. The 6- and 7- class tests can also not be relied upon, given that the 6-class model suffered issues of local maxima that could not be resolved. We take these issues to indicate that these model specifications are not well suited to the data---in line with our previous findings with subsample A that profiles were arbitrarily formed at different levels of ability.* 
...
```{r fmm3-output}
# Read in output
fmm3_lmr <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "(fmm3_lmr_)([1-8])")
...
| **c) Adjusted LRTs** (fewest classes not sig improved by additional classes)**:** 5-class, but note that this comparison is inaccurate and the 4-class model unstable. 
...
fmm3_cand <- mm_extract_data(orig_mods = m_fmm3, orig_output = fmm3_out, 
                               ...
                               filepath = "mplus_models/simpview/shapeB",
                               lrtstarts = "0 0 0 0", # bLRTs removed from process after previous issues
...
-   **Classification diagnostics:** Both models have good assignment accuracy, 5-class model has higher entropy.
-   **Interpretability:** Neither model succeeded in differentiating profiles of different shapes (i.e., dissociated strengths and weaknesses in decoding and comprehension skills); neither stand out as the key best choice. Select 5-class model on basis of fit indices.
...
```{r fmm3-selected}
fmm3_final_nclass <- 5
fmm3_final_m <- fmm3_cand$sv_fmm3_rerun_5class.out
```
...
First check that model with correlated errors for the comprehension tasks still an appropriate model for this subsample. 
...
m_cfa_fit <- mplusModeler(m_cfa,
                           modelout = "./mplus_models/simpview/shapeB/cfa_scores1.inp",
...
m_cfa1m_fit <- mplusModeler(m_cfa1m,
                           modelout = "./mplus_models/simpview/shapeB/cfa_scores2.inp",
                           check = TRUE, run = FALSE)

# Inspect model outputs
cfa_out <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "cfa")
...
m_lpa4 <- lapply(1:8, function(k) {
  ...
  # Run model
  mplusModeler(body, sprintf("mplus_models/simpview/shapeB/lpa4_%dclass.dat", k), run = FALSE)
 })

# Read in output
lpa4_out <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "(lpa4_)([1-9])")
```
...
# }
# 
# m_lpa4_starts <- lapply(c(2), function(k) {
# 
#   # Update model spec 1: overall model estimates (variances and covariances)
#   body <- update(m_base,
#               MODEL = as.formula(sprintf("~ . + '%s \n %s
...
*Note that the 7-class LMR test is not accurate (tried re-running up to 10000 starts)*
...
```{r lpa4-output}
# Read in output
lpa4_lmr <- readModels(target = "./mplus_models/simpview/shapeB", filefilter = "(lpa4_lmr_)([1-9])")
...
lpa4_cand <- mm_extract_data(orig_mods = m_lpa4, orig_output = lpa4_out, 
                               ...
                               filepath = "mplus_models/simpview/shapeB",
                               lrtstarts = "0 0 0 0",  # remove bLRT given previous issues
                               rerun = FALSE, optseed = TRUE, 
                               one_fit = TRUE)
...
-   **Classification diagnostics:** Both have good classification diagnostics, no problems with very small classes. Entropy slightly higher in higher-class model.
-   **Interpretability:** The two models show  differentiation by shape, although with less stability in profiles between models than was seen for sub-sample A. Although the 5-class model is favoured by the LMR tests, the addition of classes beyond this still show continued improvements in the fit indices. The 5-class model does not succeed in identifying a poor comprehender group---our theoretical focus---and the profiles do not correspond as well to the 5-class model fitted on subsample A. In contrast, the 6-class model shows very similar profiles to those identified initially, including a poor comprehender group, suggesting that this model is both relatively stable and well-aligned with our theoretical motivation. 
...
# Select best models
best_fits <- list(fmm3_final_m, lpa4_final_m)
...
best_summaries <- lpa_enum_table(best_fits) %>% 
  ...
  mutate(Specification = ifelse(Parameters == 61, "FMM (shape): 4 classes", # Manual relabelling as not specified above
...
all_summaries <- bind_rows( fmm3_summary, lpa4_summary) 
...
a_row <- fmm3_final_nclass
b_row <- (1*k_classes_modelled) + lpa4_final_nclass
...
summary_table %>% 
  ...
  pack_rows(index = c("Shape FMM" = k_classes_modelled, "Shape LPA" = k_classes_modelled)) %>%
  row_spec(c(a_row, b_row, bold = T))  # edit for row numbers of selected models within each model spec 

# Save table as pdf for later viewing
write.csv(summary_table, "../output/tables/shapeB_model_indices.csv", row.names = FALSE)
```

Combine the fit indices into a single plot.
...
# save out
ggsave("../output/figures/shapeB_all_indices.tiff", dpi = 600, width = 7, height = 4, units = "in")
```

## Final model

*As with the initial models on subset A, the LPA with general ability included as a covariate showed better fit to the data than the factor mixture alternative. The 6-class model successfully identified a poor comprehender group, and the profiles looked stable in comparison to initial model fitting. We are therefore reassured that the 6-class model is robust, and take this model forward to our next research question.*  

```{r final-model}
readModels("mplus_models/simpview/shapeB/sv_lpa4_rerun_6class.out", what="parameters")$parameters$r2 %>% kable("pipe")
readModels("mplus_models/simpview/shapeB/sv_lpa4_rerun_6class.out", what="parameters")$parameters$stdyx.standardized %>% kable("pipe")
```

This model is returned to the cross-validation procedure for final fitting (WP1_SimpView_ShapeValidation.Rmd).
