...
sv_data <- read.csv("../data/processed/WP1_data_subA.csv")  %>%                # ALSPAC data
  dplyr::select(yp_id, cidB3153, age_m_f8, age_m_f9, nara_comp_raw_f9, nara_acc_raw_f9, read_word_raw_f9, read_nonw_raw_f9, wold_comp_raw_f8)
```
...
m_cfa1 <- mplusObject(
  ...
  PLOT = "TYPE = PLOT3;",
  usevariables = colnames(sv_data[,!names(sv_data) %in% c("yp_id", "cidB3153")]),
  rdata = sv_data)
...
m_cfa2 <- update(m_cfa1,
                 ...
                 DEFINE = ~ "naraAcc = naraAcc/10; naraComp = naraComp/5;
                             f8age = f8age/2; f9age = f9age/2;") # two factors, plus age covariates
...
The modification indices support that model fit would be improved by estimating the covariance between the word and nonword reading accuracy tasks. 
