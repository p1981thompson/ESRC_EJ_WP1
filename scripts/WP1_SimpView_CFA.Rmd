...
sv_data <- sv_data %>% 
  ...
  # mutate(naraComp = scale(naraComp)[,],
  #        naraAcc = scale(naraAcc)[,],
  #        wordAcc = scale(wordAcc)[,],
  #        nonwAcc = scale(nonwAcc)[,],
  #        woldComp = scale(woldComp)[,])
...
m_desc_fit <- mplusModeler(m_desc,
                            dataout = "./mplus_models/desc/sv_sim.dat",
                            modelout = "./mplus_models/desc/sv_check.inp",
                            check = TRUE, run = TRUE, hashfilename = TRUE)

# Read mplus output
m_desc_out <- readModels("./mplus_models/desc/sv_check.out")
...
m_cfa1_fit <- mplusModeler(m_cfa1,
                           modelout = "./mplus_models/cfa/sv_cfa1.inp",
                           check = TRUE, run = TRUE)
...
m_cfa2 <- update(m_cfa1,
                 ...
                 MODEL = ~ "acc by nonwAcc wordAcc naraAcc; comp by woldComp naraComp;
                            wordAcc nonwAcc on f9age;
                            woldcomp on f8age;
                            acc with comp;") # two factors, plus age covariates; fixed residual var to 0 as problematic???
m_cfa2_fit <- mplusModeler(m_cfa2,
                           modelout = "./mplus_models/cfa/sv_cfa2.inp",
                           check = TRUE, run = TRUE)

# COMPARE MODELS
cfa_models <- readModels(target = "./mplus_models/cfa", filefilter = "sv_cfa")
...
m_cfa2b_fit <- mplusModeler(m_cfa2b,
                           modelout = "./mplus_models/cfa/sv_cfa2b.inp",
                           check = TRUE, run = TRUE)
