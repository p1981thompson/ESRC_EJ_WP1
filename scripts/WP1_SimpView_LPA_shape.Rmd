...
m_fmm3 <- lapply(1:6, function(k) {
  ...
  body <- update(body,
     ...
     OUTPUT = ~ . + "stdyx ENTROPY",
     SAVEDATA = as.formula(sprintf(" ~ 'FILE IS fmm3nn_%dclass.dat; SAVE = cprobabilities;'", k)))
...
# m_fmm3_starts <- lapply(4, function(k) {
# 
...
```{r fmm3-output}
# Read in output
fmm3_out2 <- readModels(target = "./mplus_models/simpview/shape", filefilter = "(fmm3_lmr_)([1-9])")

# Print table 
fmm3_summary <- lpa_enum_table(output = fmm3_out2)
fmm3_summary
...
| **b) Information heuristics** (diminishing gains from elbow plots)**:** 2- or 4-class
| **c) Adjusted LRTs** (fewest classes not sig improved by additional classes)**:** 4-class
| **d) Approximate BF** (fewest classes with moderate-strong evidence compared to previous model): 6+
...
fmm3_cand <- mm_extract_data(orig_mods = m_fmm3, orig_output = fmm3_out, 
                               ...
                               lrtstarts = "0 0 4000 1000",       # increased starts due to mplus warning
                               one_fit = TRUE)
...
-   **Interpretability:**
-   *Note that the 4-class bLRT value cannot be trusted.* 
...
m_lpa4 <- lapply(1:7, function(k) {
  ...
  #mplusModeler(body, sprintf("mplus_models/simpview/shape/lpa4_%dclass1S.dat", k), run = TRUE)
 })
...
```{r fmm3-lmr}
# # Move non-optimal models into directory (i.e., after checking using STSSCALE alternatives)
# if(dir.exists("./mplus_models/simpview/shape/nonopt")==FALSE){dir.create("./mplus_models/simpview/shape/nonopt")} 
# 
# move_file <- function(file_name = "", old_folder = "", new_folder = ""){
#   file.copy(from = paste0(old_folder, "/", file_name),
#             to = paste0(new_folder, "/", file_name))
#   file.remove(paste0(old_folder, "/", file_name))          
# }
# 
# all_files <- list.files("./mplus_models/simpview/shape/", pattern = "^lpa4.*out$")
# all_files
# nonopt <- all_files[c(2, 3, 6, 8, 10, 12)]
# 
# 
# for (file in nonopt) {
#   move_file(file_name = file, 
#             old_folder = "./mplus_models/simpview/shape",
#             new_folder = "./mplus_models/simpview/shape/nonopt")
# }
...
lpa4_summary %>% 
  select(Specification, Classes, LL, Parameters, BIC, CAIC, AWE, VLMR_p, LMR_p, BF)

# Elbow plots for information criteria
lpa_enum_elbow(lpa4_summary)# benchmark_stats = lpa_benchmark)
ggsave("../output/figures/lpa4_elbow.tiff")
```
...
| **d) Approximate BF** (fewest classes with moderate-strong evidence compared to next model): *NA/7-class*
| **e) Approximate correct model probability:** *NA/7-class*
...
# Inspect transitions 
extract_classes(lpa4_cand[3:5], type = "lpa")
```
...
-   **Interpretability:** 3-class model still mostly differentiated by level (similar for 4-class, and 4-class additionally not supported by LMR LRTs). 5-class+ models show further differentiation by shape (with some stability in profiles, i.e., PC-looking group maintained across 5- and 6-class models), but additional classes then not easy to attach verbal descriptions. 5-class best?
...
# Select best models
best_fits <- c(lpaE_final_m, fmm3_final_m, lpa4_final_m)
...
```{r lpa-all-table}
# Add summary table from best original LPA specifications (Model E, variances and covariances estimated separately for each class)
# Read in output
lpaE_lmr <- readModels(target = "./mplus_models/simpview/lpa_comp", filefilter = "(lpa_e_lmr_)([1-6])")
lpaE_summary <- lpa_enum_table(output = lpaE_lmr) 
lpaE_rerun <- readModels(target = "./mplus_models/simpview/lpa_comp", filefilter = "lpa_e_rerun") 
lpaE_summary <- add_bLRT(lpaE_rerun, lpaE_summary)

# Combine all summary tables
lpa_all_summaries <- rbind(lpaE_summary, fmm3_summary, lpa4_summary) 
...
The final model will be selected on the basis of model fit, classification utility, and theoretical interpretability. This model will be taken forward for cross-validation with subsample B (WP1_SimpView_CrossValidation.Rmd).
