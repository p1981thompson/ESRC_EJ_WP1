...
# Step 2 - Fitting latent profile model
...
```{r lpa-b}
m_lpa_b <- lapply(2:5, function(k) {
   body <- update(m_lpa_base,
     TITLE = as.formula(sprintf("~ 'LPA Model B: %d classes;'", k)),
     VARIABLE = as.formula(sprintf("~ 'classes = c(%d);'", k)),
     MODEL = ~ . + "acc with comp;")

   mplusModeler(body, sprintf("mod_scripts/sv_lpa_b_%dclass.dat", k), run = TRUE)
 })


# CHECK WORKING
lpa_b_out <- readModels(target = "./mod_scripts", filefilter = "sv_lpa_b")
(lpa_b_summaries <- mixtureSummaryTable(lpa_b_out))
```
...
m_lpa_c <- lapply(2:5, function(k) {
  ...
  body <- update(m_lpa_base,
   ...
   mplusModeler(body, sprintf("mod_scripts/sv_lpa_c_%dclass.dat", k), run = TRUE)
 })
...
m_lpa_d <- lapply(2:5, function(k) {
  ...
  # Update model
   body <- update(m_lpa_b[[1]],
     TITLE = as.formula(sprintf("~ 'LPA Model D: %d classes;'", k)),
     VARIABLE = as.formula(sprintf("~ 'classes = c(%d);'", k)),
     MODEL = as.formula(sprintf("~ . + '%s'", class_spec)))

   mplusModeler(body, sprintf("mod_scripts/sv_lpa_d_%dclass.dat", k), run = TRUE)
 })


# CHECK WORKING
lpa_d_out <- readModels(target = "./mod_scripts", filefilter = "sv_lpa_d")
(lpa_d_summaries <- mixtureSummaryTable(lpa_d_out))
```
...
m_lpa_e <- lapply(2:5, function(k) {
  ...
  # Update model
   ...
   mplusModeler(body, sprintf("mod_scripts/sv_lpa_e_%dclass.dat", k), run = TRUE)
 })


# CHECK WORKING
lpa_e_out <- readModels(target = "./mod_scripts", filefilter = "sv_lpa_e")
(lpa_e_summaries <- mixtureSummaryTable(lpa_e_out))
```
...
```{r lpa-all-mods}
lpa_all_out <- readModels(target = "./mod_scripts", filefilter = "sv_lpa_")
(lpa_all_summaries <- mixtureSummaryTable(lpa_all_out))
